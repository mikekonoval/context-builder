...
Problem: hardcoded numbers go stale. Solution — `[LIVE: slug]` marker + a separate file with the queries.

Run on demand whenever a new live number is needed in `context/business/`.
...
1. **Кладовка.** В `context/business/` валится всё без структуры (переписки, идеи, бекапы), файлы отвечают сразу на несколько тем, в подпапках с большим числом файлов нет INDEX.md.
   Check: каждый файл — одна тема? У каждой подпапки с >3 файлами есть INDEX.md?

2. **Дубль кода.** В `context/business/` описана структура таблиц БД, имена эндпоинтов, переменные окружения — то, что уже есть в коде.
   Check: есть ли описания того, что уже в коде? Должно быть только ЗАЧЕМ, не ЧТО.

3. **Хардкод цифр.** Конкретные числа (выручка, активные юзеры, цены) без маркера `[LIVE: ...]`.
   Check: какие цифры устареют через 1-3 месяца? Какие должны стать LIVE?
